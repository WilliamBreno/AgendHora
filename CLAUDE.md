...
- **Agendamento**: cliente_id, servico_id (serviço principal — ver `AgendamentoServico` abaixo pra quando há mais de um), profissional_id (referencia `Usuario`, não uma tabela `Profissional` separada), data, hora, status (`pendente` | `confirmado` | `cancelado`), pago (booleano), `valor_final` (opcional, sobrescreve o preço do(s) serviço(s) no faturamento), `valor_sinal` (opcional) + `sinal_pago` (booleano), `link_referencia` (opcional), `concluido_em` (data/hora, opcional — ver "Encaixe de horários"), `lembrete_enviado` + `lembrete_final_enviado` (controle interno dos dois lembretes automáticos — ver "Lembretes automáticos"), observacoes, estabelecimento_id; **ainda só escopo, não implementado** — `valor_antecipado` (opcional), `antecipado_pago` (booleano), `antecipado_order_nsu` (opcional) — ver "Pagamento antecipado e repasse por profissional"
- **AgendamentoServico**: liga um `Agendamento` a um serviço ALÉM do principal — ver "Agendamento com mais de um serviço". Vazio (caso mais comum) = um serviço só, comportamento de sempre
...
O cliente final pode escolher mais de um serviço pro mesmo horário (ex: corte + barba, atendidos em sequência pelo mesmo profissional) — um toggle na página pública ("Quero agendar mais de um serviço nesse horário") liga esse modo; desligado (padrão), o fluxo continua exatamente como sempre foi.

- **`AgendamentoServico`** (tabela nova): vincula os serviços ALÉM do principal a um `Agendamento` — o principal continua em `Agendamento.ServicoID`/`Servico`, como sempre. `Agendamento.TodosServicos()` devolve a lista completa (principal primeiro); usar sempre que duração ou preço precisarem considerar o conjunto inteiro, nunca só `Servico` direto
- **Duração**: soma sequencial da duração efetiva (`Servico.DuracaoEfetivaMin()`) de cada serviço — bloqueia a agenda como um intervalo contínuo só, do mesmo jeito que já bloqueia pelo teto da faixa em duração variável
- **Preço**: soma o preço de cada serviço; se QUALQUER um deles estiver sem preço cadastrado ("a combinar"), o total inteiro vira "a combinar" — nunca mostra uma soma parcial que sabemos que está incompleta. `Agendamento.ValorFinal` continua funcionando como sempre, sobrescrevendo esse total calculado (ex: o dono negocia um valor fechado pro combo inteiro)
- **Compatibilidade com serviços individuais**: um serviço individual (ver "Serviços individuais") só combina com o catálogo geral e com outros serviços individuais do MESMO profissional — nunca dois profissionais diferentes no mesmo agendamento. A página pública já filtra a lista de "adicionar mais um" pra só mostrar o que é compatível (o cliente nunca consegue montar uma combinação impossível); o backend valida de novo na criação, defesa em profundidade
- **Ranking do dashboard**: cada serviço de um combo conta 1 na quantidade; o faturamento é rateado proporcionalmente ao preço de catálogo de cada um (ou em partes iguais se nenhum tiver preço) — nunca conta o valor cheio do agendamento pra cada serviço, o que inflaria o total somado do ranking
- **Agenda, e-mails e relatórios**: a pílula na agenda usa a cor do serviço principal com um "+N" ao lado do nome quando há mais; painel de detalhe, e-mails de confirmação/lembrete/cancelamento, histórico de atividades e exportações (CSV/XLSX/PDF) sempre listam todos os serviços por extenso ("Corte + Barba")
- **Reagendamento automático** (ver seção própria): reaproveita a duração total do último agendamento pra procurar o próximo horário livre, e o link do e-mail carrega todos os `servico_id` do combo original (repetindo o parâmetro na query string)
- **Admin também cria agendamento múltiplo**: o diálogo "Novo agendamento" do painel (agendamento feito por telefone/balcão) tem o mesmo toggle e a mesma regra de compatibilidade da página pública — escolhido o serviço principal, o dono/auxiliar pode marcar serviços adicionais compatíveis antes de salvar. Um serviço individual no combo já preenche o profissional sozinho (o campo de escolha some, vira só um texto informativo), igual a página pública pula a etapa de escolher profissional
...
- **Autocompletar de cliente**: os campos "Nome do cliente" no registro manual de venda de produto e na criação manual de agendamento pelo admin (`NovoAgendamentoDialog`) buscam (sem acento, sem diferenciar maiúsculas) entre os clientes já cadastrados enquanto o dono/auxiliar digita — clicar numa sugestão preenche nome e telefone de uma vez, sem digitar o contato de quem já tem cadastro. Busca client-side (a lista de clientes do estabelecimento já é pequena o bastante pra carregar inteira), componente reutilizável `ClienteAutocomplete`
...
- **Histórico de vendas**: aba "Vendas" na própria tela Produtos (ao lado de "Catálogo") lista todas as vendas já registradas — filtro "Todas" / "Clientes" / "Equipe" (é aqui que o dono vê o que cada profissional já comprou internamente, com desconto ou não), com ação de marcar como pago e cancelar (devolve o estoque) direto na lista, mesma regra de sempre. Antes dessa aba existir, o backend já listava tudo (`GET /vendas-produtos`) mas não havia tela nenhuma pra ver — só o diálogo de registrar
...
## Pagamento antecipado e repasse por profissional (escopo)

Pedido do dono do projeto: dar ao cliente final a opção de já garantir o horário pagando na hora do agendamento (em vez de só reservar e torcer pra não dar no-show), e dar a cada profissional visibilidade de quanto vai receber daquele atendimento específico. **Só o desenho está fechado aqui — nada disso foi implementado ainda.** Serve de roteiro pra quando entrar em construção, do mesmo jeito que "Integração futura com WhatsApp" documentou o WhatsApp antes de existir código — com a diferença de que aqui não há bloqueio externo (a InfinitePay já está integrada), então a implementação pode começar assim que esse desenho for aprovado.

### Pagamento antecipado

- **Decisão fechada**: o modo de cobrança é **por estabelecimento inteiro**, não por serviço — um interruptor só em Configurações vale pra qualquer agendamento feito ali. `Estabelecimento.modo_pagamento_antecipado`: `"nenhum"` (padrão, comportamento de sempre) | `"metade"` | `"total"` | `"porcentagem"`. Com `"porcentagem"`, `Estabelecimento.percentual_pagamento_antecipado` (inteiro, 1–100) define quanto
- **Só cobra quando o preço é conhecido**: se o total do agendamento (soma dos serviços, ou `valor_final` quando existir) for "a combinar" (ver "Segmentos de negócio"), a cobrança antecipada não se aplica àquele agendamento — não dá pra calcular metade/porcentagem de um valor indefinido. Segue o fluxo normal, sem cobrança
- **O agendamento nasce `confirmado` do mesmo jeito de sempre** (ver "Fluxo de confirmação do agendamento") — pagamento antecipado não é aprovação nem trava de criação, é uma cobrança que acontece **depois** de o horário já estar reservado. Mantém a decisão já fechada de que ninguém precisa aprovar manualmente
- **Fluxo proposto**: ao confirmar na página pública, se `modo_pagamento_antecipado != "nenhum"` e o total é conhecido, o backend calcula o valor (metade = 50% do total, total = 100%, porcentagem = `percentual_pagamento_antecipado`) e gera um link via InfinitePay Checkout Integrado — mesma API já usada pra assinatura (ver "Cadastro e ativação de novos estabelecimentos"), com `order_nsu` próprio (ex.: `agendamento-<id>`, prefixo que distingue do `order_nsu` de assinatura/renovação no mesmo webhook `POST /webhooks/infinitepay`). A tela de confirmação mostra "Agendamento confirmado — falta pagar R$X pra garantir" com o link, em vez de (ou além de) o aviso de sucesso de sempre
- **Webhook confirma o pagamento**: mesma rota já existente, só reconhecendo o prefixo do `order_nsu` — marca `Agendamento.antecipado_pago = true` e dispara uma notificação extra de "pagamento confirmado" pro cliente e pro dono
- **Não pagar não cancela o horário** (proposto, a confirmar): o agendamento já nasce confirmado e o horário já bloqueia a agenda assim que criado — quem não paga continua com o agendamento válido, o pagamento antecipado funciona como incentivo/garantia extra, não trava. Cancelamento automático por falta de pagamento (expira em N minutos e libera o horário) é um comportamento mais rígido que pode virar uma configuração opcional depois, mas não faz parte deste escopo inicial
- **Cancelamento com valor já pago**: reembolso automático via InfinitePay não está confirmado como possível — não há endpoint de estorno documentado no mesmo painel do Checkout Integrado (mesma situação já registrada em "Avaliado e descartado" sobre Planos e assinaturas). Até confirmar isso, cancelar um agendamento com `antecipado_pago = true` é **manual**: o admin vê o valor pago no painel de detalhe e decide o reembolso fora do sistema (Pix direto, por exemplo) — mesmo espírito de "Comunicado de aniversário é manual" já adotado em outras partes
- Cliente final **só vê o preço integral do agendamento** — nunca vê percentual de comissão nem o valor líquido que o profissional recebe; o valor antecipado a pagar é sempre calculado sobre esse preço cheio

### Repasse por profissional (comissão)

- **Decisão fechada**: percentual **padrão do estabelecimento, sobrescrevível por profissional**. `Estabelecimento.percentual_comissao_padrao` (opcional, inteiro 0–100, em Configurações) vale pra todo profissional sem valor próprio; `Usuario.percentual_comissao` (opcional, inteiro 0–100, editado na tela Equipe) sobrescreve só aquele profissional quando preenchido. Sem nenhum dos dois preenchidos, não há repasse calculado (mostra só o valor cheio, como hoje)
- **Base de cálculo**: o mesmo valor já usado no faturamento — `Agendamento.ValorFinal` quando existir, senão a soma do(s) serviço(s) (`PrecoTotal()`, ver "Agendamento com mais de um serviço"). Repasse = base × percentual do profissional responsável (`Agendamento.ProfissionalID`). Escopo é só `Agendamento` — venda de produto (`VendaProduto`) não entra nessa comissão, já tem seu próprio cálculo separado (desconto da equipe, ver "Cadastro de produtos")
- **Painel de detalhe do agendamento**: mostra "Repasse: R$X (Y%)" pro profissional responsável — visível pro dono (qualquer agendamento) e pro próprio profissional (só nos dele, mesma regra de sempre de "auxiliar só vê o que é dele"). Nunca aparece pro cliente final, em nenhum canal (página pública, e-mail, "meus agendamentos")
- **Dashboard**: `Estabelecimento.mostrar_repasse_dashboard` (booleano, padrão `false`, em Configurações) decide se os cards/gráficos de faturamento mostram o valor **líquido** (descontando o repasse de cada profissional — o que sobra pro estabelecimento) ou o valor **cheio** (bruto, comportamento de hoje). Combinado com o filtro de profissional já existente no Dashboard, o dono também consegue ver quanto cada profissional individualmente já rendeu de repasse no período

### Novos campos (resumo)

- **`Estabelecimento`**: `modo_pagamento_antecipado`, `percentual_pagamento_antecipado` (opcional), `percentual_comissao_padrao` (opcional), `mostrar_repasse_dashboard` (booleano)
- **`Usuario`**: `percentual_comissao` (opcional) — sobrescreve o padrão do estabelecimento só pra esse profissional
- **`Agendamento`**: `valor_antecipado` (opcional, valor calculado/cobrado), `antecipado_pago` (booleano), `antecipado_order_nsu` (opcional) — pra casar a confirmação do webhook

### Em aberto (decidir antes de implementar)

- Reembolso automático de valor antecipado em cancelamento — depende de confirmar se a InfinitePay expõe algum endpoint de estorno pro Checkout Integrado; até lá, é manual
- Cancelamento automático por falta de pagamento (expiração do horário reservado sem pagar) — proposto ficar de fora desta v1, registrado como possível extensão futura
- Se o lembrete automático (3h/30min antes) deve reforçar "ainda falta pagar R$X" quando `antecipado_pago = false` — nice-to-have, não decidido
...
21. Agendamento com mais de um serviço: toggle na página pública, tabela `AgendamentoServico`, duração somada bloqueando a agenda como intervalo único, preço somado (ou "a combinar" se algum serviço não tiver preço), filtro de compatibilidade com serviços individuais, ranking do dashboard rateado por serviço; mesmo toggle disponível no "Novo agendamento" do admin
22. Pagamento antecipado e repasse por profissional — **ainda só escopo desenhado** (ver seção própria), não implementado: modo de cobrança por estabelecimento (`nenhum`/`metade`/`total`/`porcentagem`), link InfinitePay gerado por agendamento, comissão padrão sobrescrevível por profissional, exibição de repasse no painel de detalhe e no dashboard (bruto ou líquido, à escolha do dono), nada disso visível pro cliente final
...
- Agendamento com mais de um serviço só combina serviços do mesmo profissional (catálogo geral + individuais dele) — nunca dois profissionais diferentes no mesmo horário; preço total vira "a combinar" se qualquer serviço do combo não tiver preço, nunca uma soma parcial
- Pagamento antecipado e repasse por profissional ainda é só escopo (ver seção própria) — quando entrar em construção: o modo de cobrança é por estabelecimento inteiro (não por serviço); o percentual de comissão é um padrão do estabelecimento sobrescrevível por profissional (nunca obrigatório por profissional); e o cliente final nunca vê percentual de comissão nem valor líquido, só o preço integral
